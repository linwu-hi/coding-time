import{_ as n,o as s,c as a,e}from"./app-0301958d.js";const p={},t=e(`<h1 id="滑动窗口算法" tabindex="-1"><a class="header-anchor" href="#滑动窗口算法" aria-hidden="true">#</a> 滑动窗口算法</h1><p>滑动窗口算法是一种常用的解决数组/列表问题的方法，特别是对于子字符串或子数组问题。这种技巧可以用来解决多种类型的问题，包括最长子串、最小子串、数组和等问题。</p><p>滑动窗口的基本思想是维护一个窗口，通常在数组或列表上滑动，用于观察该窗口内的元素。窗口的大小、滑动的方式（左滑、右滑或同时）取决于具体问题。</p><p>以下是一个滑动窗口技巧的通用算法框架：</p><ol><li><p>定义两个指针，一个用于表示窗口的左端，另一个表示窗口的右端。初始化都指向数组的开始位置。</p></li><li><p>将右指针向右移动，扩大窗口，直到窗口内的子数组满足问题的条件。</p></li><li><p>当窗口满足条件时，尝试将左指针向右移动，缩小窗口，同时保持窗口仍满足条件。</p></li><li><p>记录每次更新窗口时的有用信息。例如，如果你在寻找最长子串，那么应记录满足条件的最长长度。</p></li><li><p>将步骤2-4作为循环，直到右指针到达数组的末尾。</p></li></ol><div class="language-javascript line-numbers-mode" data-ext="js"><pre class="language-javascript"><code><span class="token keyword">function</span> <span class="token function">slidingWindow</span><span class="token punctuation">(</span><span class="token parameter">arr<span class="token punctuation">,</span> handleWindow</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
